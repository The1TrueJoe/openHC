---
title: HDMI CEC
description: Why CEC is absent from every layer of the HDMI path, and the one place it does exist.
sidebar:
  order: 11
---

The CE5310 integrates HDMI 1.4a, and 1.4a includes CEC. So the question isn't
whether the silicon can do it. It's whether anything between the pin and
userspace is wired up.

On the EA1 the answer is **no, at every layer**, and the one place CEC does appear
is somewhere nobody would guess.
...
| `/proc/config.gz` | no cec symbol — the kernel is 3.12, and mainline's CEC framework landed in 4.8 |
| `libgdl.so` (host and container copies) | no cec symbols, no cec strings |
| `pd_hdmi.ko`, `gdl_server.ko` | nothing — and 377 `hdmi` strings in `pd_hdmi`, so the search was working |
...
The decisive one is the port driver's own attribute table. `portattrs -port HDMI
-dump` returns 42 attributes, and every single one is TMDS, HDCP, DDC or EDID —
cable status, EDID retry, I²C bus reset, power. **There's no CEC attribute.**
Whatever Intel wired into this port driver, CEC was not part of it.
...
:::note[Why reading the EDID used to hang]
...
The fourth argument is a size **by value**. Passing a `uint32_t*` there, which is
what the first attempt did, assuming it was an in/out length — makes the call
never return. The other half: the buffer is 129 bytes and **bidirectional** —
byte 0 is the block index on the way in, bytes 1..128 are the block on the way
back. Port id is 2 and recv id is 2; neither needs discovering.
...
8051 inside the SoC that CEFDK reports at boot. Intel's own diagnostic tool
settles that, and also confirms the port assignment is Intel's rather than
Control4's:
...
`intel_pic_uart.ko` and its userspace half `libpicuart.so` are likewise Intel's,
and both carry a full CEC message class:
...
Header nibbles, an opcode, up to fourteen operands and a broadcast flag. **That's
...
process owns it — Control4's `watchdogd` — and it's feeding a hardware watchdog
that resets the board when the heartbeat stops. **A second writer on that port is
a reboot.**

There's a zero-risk way to listen, though. `watchdogd` logs every message the PIC
sends it, and logs `Unknown cmd(N)` for anything it does not handle. Over ten and
a half minutes with a powered, CEC-capable TV attached: **66 messages, all
...
continuously, but combined with the total absence of CEC anywhere else, it points
one way.
...
software. There's no PIC24 image anywhere on the box either —
`/control4/firmware/` holds images for *peripherals*, not for the controller's own
...
**A USB CEC adapter.** `CONFIG_USB_ACM=y` is built into the kernel and char major
166 is registered, so a Pulse-Eight style adapter enumerates with no kernel work
at all. libcec talks to it over the resulting `/dev/ttyACM0`. **This is the only
option that does not depend on an unverified hardware fact.**

**IR instead.** The TM4C already drives five IR outputs and the transmit path
[works](/shared/io-mcu/#ir-output). This is also, in practice, how Control4
turns televisions on. Not CEC, but it solves the actual problem.

**Own the PIC link.** Replace `watchdogd` with a daemon that owns `/dev/ttyS2` and
multiplexes the heartbeat with everything else the PIC can do — CEC,
`setGPIOValue`, `setPWM`, `setIrRepeatMode`. That's the only route to native CEC
on this hardware, it's gated behind both unverified facts above, and getting the
heartbeat wrong reboots the box. It is worth doing for its own reasons; CEC would
be a side effect, not the justification.

## Cost of finding this out

One reboot. A recursive `grep` across `/usr /lib /bin /sbin` ran long enough to
starve `watchdogd`, and the PIC reset the board. **Ten minutes of sustained
filesystem work is apparently enough.**
