...
/*
 * ohc-splash — draw the openHC banner straight onto the framebuffer.
 *
 ...
 * the usual invocation is simply:  figlet -f slant openHC | ohc-splash -
 */
...
{
	const char *path = argc > 1 ? argv[1] : "/etc/ohc-splash.txt";
	...
	while (nrows && strspn(rows[nrows - 1], " ") == strlen(rows[nrows - 1]))
		nrows--;
	if (!nrows || !cols) { fprintf(stderr, "ohc-splash: empty banner\n"); return 1; }
	...
	if (var.bits_per_pixel != 32) {
		fprintf(stderr, "ohc-splash: need 32bpp, got %u\n", var.bits_per_pixel);
		close(fd); return 1;
	}
	 ...
	 * a knob rather than a constant -- OHC_SPLASH_FILL overrides it, and
	 * S01splash passes it through. */
	fill = 42;
	{
		const char *e = getenv("OHC_SPLASH_FILL");
		if (e) {
			int v = atoi(e);
			if (v >= 5 && v <= 100) fill = v;
			else fprintf(stderr, "ohc-splash: ignoring OHC_SPLASH_FILL=%s "
			                     "(want 5..100)\n", e);
		}
	}
