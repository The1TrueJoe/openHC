...
echo "post-image:   ssh root@<ip> 'mkdir -p /mnt/k && mount /dev/sda3 /mnt/k'"
echo "post-image:   cat $KERNEL_NAME | ssh root@<ip> 'cat > /mnt/k/boot/$KERNEL_NAME'"
echo "post-image:   cat $INITRD_NAME | ssh root@<ip> 'cat > /mnt/k/boot/$INITRD_NAME'"
echo "post-image:   ssh root@<ip> 'umount /mnt/k'"
echo "post-image:"
...
echo "post-image:   ssh root@<ip> 'mkdir -p /mnt/g && mount /dev/sda1 /mnt/g &&"
echo "post-image:                   cp /mnt/g/boot/grub/menu.lst /mnt/g/boot/grub/menu.lst.stock'"
echo "post-image:   cat menu.lst.openhc | ssh root@<ip> 'cat >> /mnt/g/boot/grub/menu.lst'"
echo "post-image:   ssh root@<ip> 'sed -i \"s/^default.*/default\t\t2/\" /mnt/g/boot/grub/menu.lst &&"
