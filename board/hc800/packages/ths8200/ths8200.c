...
int main(int argc, char **argv)
{
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        if (r >= MAX_REGS || v > 0xff) {
            fprintf(stderr, "ths8200: %s: bad entry %x %x\n", path, r, v);
            fclose(f);
            return 1;
        }
    ...
    if (count == 0) {
        fprintf(stderr, "ths8200: %s contains no registers\n", path);
        return 1;
    }
    printf("ths8200: %d registers from %s\n", count, path);

    if (dry) {
        printf("ths8200: dry run, hardware untouched\n");
        return 0;
    }
    ...
    fd = open(dev, O_RDWR);
    if (fd < 0) {
        fprintf(stderr, "ths8200: %s: %s (CONFIG_I2C_CHARDEV?)\n",
                dev, strerror(errno));
        return 1;
    }
     ...
     * SMBus-only is exactly what made this fail the first time. */
    {
        ...
        if (ioctl(fd, I2C_FUNCS, &funcs) == 0) {
            printf("ths8200: %s funcs: raw-I2C=%s SMBUS_BYTE_DATA=%s\n", dev,
                   (funcs & I2C_FUNC_I2C) ? "yes" : "no",
                   (funcs & I2C_FUNC_SMBUS_BYTE_DATA) ? "yes" : "no");
            if (!(funcs & I2C_FUNC_SMBUS_BYTE_DATA)) {
                fprintf(stderr, "ths8200: adapter cannot do SMBus byte-data\n");
                close(fd);
                return 1;
            }
        }
    ...
    if (ioctl(fd, I2C_SLAVE, addr) < 0) {
        fprintf(stderr, "ths8200: address 0x%02x: %s\n", addr, strerror(errno));
        close(fd);
        return 1;
    }
    ...
    if (i2c_read_reg(fd, THS8200_VERSION_REG, &ver) < 0) {
        fprintf(stderr, "ths8200: no answer at 0x%02x on %s\n", addr, dev);
        close(fd);
        return 1;
    }
    if (ver != THS8200_VERSION_VAL) {
        fprintf(stderr, "ths8200: VERSION reads 0x%02x, expected 0x%02x — "
                        "not a THS8200, refusing to write\n", ver, THS8200_VERSION_VAL);
        close(fd);
        return 1;
    }
    printf("ths8200: THS8200 at %s:0x%02x, VERSION 0x%02x\n", dev, addr, ver);
    ...
    for (int r = 0; r < MAX_REGS; r++) {
        ...
        if (i2c_write_reg(fd, (unsigned char)r, regs[r]) < 0) {
            fprintf(stderr, "ths8200: write 0x%02x=0x%02x failed: %s\n",
    ...
    close(fd);
    printf("ths8200: wrote %d registers, output configured\n", written);
    return 0;
}
