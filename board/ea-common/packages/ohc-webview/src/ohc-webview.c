/*
 * ohc-webview -- a full-screen web surface for openHC.
 *
...
/*
 * The box already serves a UI: ohc-webd listens on :80 (S90ohcweb) and
 ...
 * rather than to a blank page. OHC_WEBVIEW_URL overrides, which is how a
			       ...
			       const char *uri, GError *err, gpointer user)
{
	 ...
	 */
	g_printerr("ohc-webview: load failed: %s: %s\n", uri,
...
int main(int argc, char **argv)
{
	...
	if (!backend) {
		g_printerr("ohc-webview: no libwpe backend "
	 ...
	 *
	 * OHC_WEBVIEW_NOACCEL=1 forces it off. That is a bring-up escape
